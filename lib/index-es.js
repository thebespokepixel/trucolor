...
import { TinyColor, names, tinycolor } from '@thebespokepixel/es-tinycolor';
...
var parse = function (color) {
	...
	while (tokens.length > 0) {
		...
		switch (token) {
			...
			case 'softlight':
			case 'soft':
				processor$$1.softlight({
					color: tokens.shift()
				});
				break;
			case 'hardlight':
			case 'hard':
				processor$$1.hardlight({
					color: tokens.shift()
				});
				break;
			case 'difference':
			case 'diff':
				processor$$1.difference({
					color: tokens.shift()
				});
				break;
			case 'exclusion':
			case 'excl':
				processor$$1.exclusion({
					color: tokens.shift()
				});
				break;
			case 'average':
			...
			default:
				if (/^[A-Za-z0-9_-]+:$/.test(token)) {
					if (processor$$1.hasSource) {
						processor$$1 = processor(`color_${ currentAutoName++ }`);
					}
					processor$$1.lock(token.trim());
				} else {
					if (processor$$1.hasSource) {
						processor$$1 = processor(`color_${ currentAutoName++ }`);
					}
...
export { console, pkg, metadata, trucolor, palette, simple, chalkish };
