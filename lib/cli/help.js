...
/*
 ...
 Command line help
 */
var _pages, _trucolor, _truwrap, clr, console, converter, deepAssign, header, imageSpacer, img, spectrum, synopsis, terminalFeatures;
...
_pages = {
  ...
  named: {
    usage: "Named Colors:\n",
    examples: function(width_) {
      var ansi, c, cell, cellsrc, color_, colors, columns, colwidth, grid, i, j, name_, namedColors, ref, ref1, table;
      namedColors = require("../named");
      colors = (function() {
        var results;
        results = [];
        for (name_ in namedColors) {
          color_ = namedColors[name_];
          results.push({
            name: name_,
            color: color_
          });
  ...
  epilogue: "" + clr.title + (_trucolor.getName()) + clr.normal + " is an open source component of CryptoComposite\'s toolset.\n" + clr.title + "© 2015 CryptoComposite. " + clr.grey + "Released under the MIT License." + clr.normal + "\n"
};
