...
setupPage = function(clr) {
  ...
  if (terminalFeatures.color.has16m) {
    ...
    header = function() {
      return [" " + clr.red + "━┳━╸     " + clr.bright + "╭──╮  ╷", img.space + " " + clr.green + " ┃ ┏━┓╻ ╻" + clr.bright + "│  ╭─╮│╭─╮╭─╮ " + clr.normal + (trucolor.getDescription()), img.space + " " + clr.blue + " ╹ ╹  ┗━┛" + clr.bright + "╰──╰─╯╵╰─╯╵   " + clr.grey + (trucolor.getVersion(3))].join("\n");
    };
  } else {
    header = function() {
      return ["" + clr.bright + (trucolor.getName()) + clr.normal, "" + img.space + (trucolor.getDescription()), "" + img.space + clr.grey + (trucolor.getVersion(3)) + clr.normal].join("\n");
    };
    spectrum = function() {
